...
class StatusActionsViewModelTest {
    ...
    private lateinit var viewModel: StatusActionsViewModel
    ...
    fun setup() {
        api = mock()
        eventHub = EventHub()
        viewModel = object : StatusActionsViewModel(api, eventHub) {}
    }
    ...
    @Test
    fun `pin failure with server error shows snackbar`() = runTest {
        api.stub {
        ...
        viewModel.snackbarErrors.test {
            viewModel.pin(statusId, true)
            val snackbarError = awaitItem()
